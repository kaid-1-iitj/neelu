...
router.post("/signup", async (req, res) => {
  ...
  if (body.role === "Agent") {
    return res
      .status(403)
      .json({ error: "Agents must be created by an Admin" });
  }
...
router.post(
  "/bills",
  requireAuth(["Admin", "Manager", "Treasurer", "Secretary", "President"]),
  async (req, res) => {
    const Schema = z.object({
      ...
      attachments: z
        .array(z.object({ fileName: z.string(), fileURL: z.string().url() }))
        .optional(),
...
router.get(
  ...
  async (req, res) => {
    const { societyId, status, q } = req.query as {
      societyId?: string;
      status?: string;
      q?: string;
    };
...
router.put(
  ...
  async (req, res) => {
    ...
    }) satisfies z.ZodType<UpdateBillStatusRequest>;
    ...
    await col.updateOne(
      { _id: id },
      { $set: { status }, $push: { remarks: newRemark as any } },
    );
    return res.json({ ok: true });
  },
);
...
router.post(
  "/bills/:billId/remarks",
  requireAuth(["Agent", "Manager", "Treasurer", "Secretary", "President"]),
  async (req, res) => {
    const id = toId(req.params.billId);
    if (!id) return res.status(400).json({ error: "Invalid id" });
    const Body = z.object({
      text: z.string().min(1),
    }) satisfies z.ZodType<CreateRemarkRequest>;
